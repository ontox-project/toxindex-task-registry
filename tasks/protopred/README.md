...
- Model catalog & parameter sync to API format  
    1) Catalog: `MODEL_CATALOG` maps every module/property/model; `list_models(module=None)` exposes it.  
    2) Resolution: flexible names (`logp`, `water`, `model_phys:water_solubility`, etc.) are normalized to canonical `model_<prop>:<name>` (deduped, order-preserved).
